...
#         name=data['name'],
#         email=data['email'],
#         password=data['password'],  # In real apps, hash it!
...
#     # Get state for this response session
...
#     # Always lock the test after submission
...
#     response = Response(user_id=user_id, quiz_id=quiz_id)
...
# @app.route('/api/quiz/<int:quiz_id>/responses', methods=['GET'])
...
#         return jsonify({"error": "Only admin can view results"}), 403
...
#         db.create_all()  # ✅ Creates all missing tables in the DB
#     app.run(debug=True)
